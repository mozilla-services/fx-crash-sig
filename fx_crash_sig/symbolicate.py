...
class Symbolicator:
    ...
    def __get_symbolicated_trace(self, sym_request):
        response = requests.post(
            self.api_url, headers={"User-Agent": "fx-crash-sig/1.0"}, json=sym_request
        )
    ...
    def symbolicate_multi(self, traces):
        ...
        try:
            ...
            if self.verbose:
                print(
                    "fx-crash-sig: Failed Symbolication: {}".format(e), file=sys.stderr
                )
                print(
                    "fx-crash-sig: Request payload was: {}".format(
                        symbolication_requests
                    ),
                    file=sys.stderr,
                )
                print(
                    "fx-crash-sig: Got response: {}".format(e.response.text),
                    file=sys.stderr,
                )
            return None
